 ...
 */
export function createSimpleProcessManager(context, name, eventTypes, handlerFn) {
    debugProcess('[createSimpleProcessManager] Creating process manager: %s for event types: %s', name, eventTypes.join(', '));
    return reactor({
        ...
        eachMessage: async (message) => {
            ...
            try {
                ...
                debugProcess('[createSimpleProcessManager:%s] Error processing event %s: %s', name, message.type, error instanceof Error ? error.message : String(error));
                throw error;
            }
        },
...
//# sourceMappingURL=simple-process-manager.js.map